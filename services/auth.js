...
const { JWT,LOGIN_ACCESS,
    PLATFORM,MAX_LOGIN_RETRY_LIMIT,LOGIN_REACTIVE_TIME,FORGOT_PASSWORD_WITH} = require("../constants/authConstant");
...
isActive: true,isDeleted: false,        }
        ...
        if(FORGOT_PASSWORD_WITH.LINK.email){
            ...
            let mailObj = {
                ...
                to: user.email,
                template: "/views/email/ChangePassword",
                data:{
                    username:updatedUser.,
                    ileti:updatedUser.updatedBy,
                }
            };
            try{
...
where.isActive = true;where.isDeleted = false;        const dbUser = await dbService.findOne(model.user,where);
        ...
        let updatedUser = await dbService.update(model.user, where, {
        ...
        await dbService.update(model.userAuthSettings, {userId:userId}, { resetPasswordCode: '', expiredTimeOfResetPasswordCode: null, loginRetryLimit: 0 })
        let mailObj = {
            subject: 'Reset Password',
            to: dbUser.email,
...
module.exports = {
    loginUser,changePassword,sendResetPasswordNotification,resetPassword,sendPasswordBySMS,sendPasswordByEmail
}